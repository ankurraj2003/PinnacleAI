...
PROMPT = """You are the Trend Detection Agent for Pinnacle Equity Group.
...
def create_trend_agent() -> AgentExecutor:
...
async def run_trend_detection(company_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        result = agent.invoke({"input": f"Detect trends for company '{company_id}' across revenue, gross_margin, ebitda_margin, opex_pct over 36 months. Use the fit_linear_trend tool. Flag concerning trends."})
